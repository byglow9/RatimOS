...
### UI Design System — Spacing Scale
- **D-16:** Phase 1 formally adopts (does not invent) the pre-existing Phase-0 micro-spacing scale — 4/6/8/10px, plus 16/24 for larger layout breaks — as this project's **locked spacing convention**, overriding GSD's generic 8-point-scale default for this project. Rationale: the physical target is a 320×480 3.5" panel, where 8px-multiple increments consume proportionally more of the available chrome (topbar/sectionbar icon-to-label gaps, bar heights) than the same increments would on a desktop/mobile web canvas — the existing `theme.h`/`.c` constants (6px sectionbar gap, 10px screen-edge padding) already encode this in Phase 0 code. Reworking these constants now is explicitly out of scope for Phase 1 (a plumbing/shell phase); any future visual-identity rework belongs to Phase 9 (VISUAL-01). This is logged here as a project-level convention override — added mid-Phase-1 during UI-SPEC verification, not part of the original context-gathering session — so all future phases' UI-SPECs inherit the same locked scale rather than re-litigating it. **Reversibility:** low-cost to revisit later (Phase 9), high-cost to change now (would touch already-shipped Phase 0 theme constants for no functional gain in a plumbing phase).

### UI Design System — Visual Identity Palette (brought forward from Phase 9)
- **D-17:** The user supplied the official RatimOS logo (`logo/RatimOS.png` at project root) — a pixel-art/16-bit-style wordmark with a chess-rook/tower-and-flag mark, black background, and a red↔orange↔purple gradient treatment with beveled pixel-art shading. The user explicitly chose to **repaint the whole UI-SPEC palette now, in Phase 1**, rather than deferring the palette work to Phase 9 (VISUAL-01) as the roadmap originally sequenced. This supersedes the Phase 1 UI-SPEC's previously-approved palette (`#14102b` bg / `#2b2358`+`#4a3f8f` panels / `#6dd6c4` teal-mint accent) — the new palette must be derived from this logo asset (dark/near-black dominant background, red/orange/purple accent family) and applied across all 5 app screens and the shell, not just the splash. Phase 9 (VISUAL-01) scope is narrowed accordingly to whatever visual-identity work remains after this (bespoke icon set, final games polish) — the palette/color-identity itself is now Phase 1 scope. **Reversibility:** low — this is the first real palette decision made from an actual brand asset (vs. an arbitrary Phase-0 placeholder palette), so it is expected to hold through to delivery rather than be revisited again in Phase 9.
- The splash screen should render the actual `logo/RatimOS.png` artwork (converted to an LVGL image asset) rather than a plain text label, since Phase 1 already owns the boot identity/splash (SHELL-03).
...
- `logo/RatimOS.png` — official RatimOS logo/wordmark (see D-17). Source of truth for the new visual-identity palette adopted in this phase; also the intended splash-screen artwork.
