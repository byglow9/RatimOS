...
static const char s_termo_answers[][RATIMOS_TERMO_WORD_LEN + 1] = {
    "abril",
    "abrir",
    "abuso",
    "acaso",
...
static const char s_termo_accepted[][RATIMOS_TERMO_WORD_LEN + 1] = {
    "ababa",
    "abaca",
    "abaco",
    "abada",
...
{
    return sizeof(s_termo_answers) / sizeof(s_termo_answers[0]);
}

const char * ratimos_termo_answer_at(size_t index)
{
    if (index >= ratimos_termo_answer_count()) {
        return NULL;
    }
    return s_termo_answers[index];
}
...
bool ratimos_termo_is_accepted_guess(const char * word)
{
    ...
    for (size_t i = 0; i < RATIMOS_TERMO_WORD_LEN; i++) {
    ...
    size_t lo = 0;
    size_t hi = sizeof(s_termo_accepted) / sizeof(s_termo_accepted[0]);
    while (lo < hi) {
        size_t mid = lo + (hi - lo) / 2;
        int cmp = strcmp(lower, s_termo_accepted[mid]);
        if (cmp == 0) {
            return true;
        } else if (cmp < 0) {
            hi = mid;
